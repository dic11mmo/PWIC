function Transmitting_Signa = TX( In_puts ) %Parameters description
Sampling_Freq = 44100;
Carrier_freq = 1500; %carrier frequency %Nbr_Ofbits=2^10;
...
Complex_sig=Re+1i*Im; % QAM Symbols %1. Mapping bits to symbols
% 2. Generating pilots
...
%7.Finding the CPs which is the last 20 values of each Nsc placed at the %beginning)
Pilot = Paralle_To_Serial(1:1:128);
 Paralle_To_Serial_WithPilot= Paralle_To_Serial(129:1:end); for l=1:length(Paralle_To_Serial_WithPilot)/Nsc
CP(l,:)=Paralle_To_Serial_WithPilot(l*Nsc-19:1:l*Nsc); end
%8.The CPs inserts 
