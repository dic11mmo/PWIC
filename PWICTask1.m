...
Ts = 2.3*10^-3;
Beta = (1/100);
sample_time = 1/Sampling_Freq; N = round(Ts*Sampling_Freq); Nbr_Ofbits=1000;
mu=0.00023;
...
%%%%%%%%%%                      Modulation     %%%%%
t1=0:1/Sampling_Freq: ((length(Real_PulsTrain)-1)/Sampling_Freq); RQ=sqrt(2).*Real_PulsTrain.*cos(2*pi*Carrier_Freq*t1);
 
RI=-Img_PulsTrain.*sqrt(2).*sin(2*pi*Carrier_Freq*t1); Modulat= RQ + RI;
...
%%%%%   QPSK_Demodulation %%%%%%%%%
RQ = Signal_With_Noise.*cos(2*pi*Carrier_Freq*t1);
RI =-Signal_With_Noise.*(sin(2*pi*Carrier_Freq*t1)); 
Demodulat = RQ +RI;
...
Rec_signal_Qpart=Match_filter_RQ(1:N:end); % we get a vector of values from 1 then every 101:e
...
Rec_signal_Ipart=(Match_filter_RI(1:N:end));
Rec_signal_Ipart_Img=1i*Rec_signal_Ipart;
% sign gives us +-1 vectors we start from 2
% because we have zero in the starting of the vector
...
% ylabel('BER');
figure(5)
semilogy(SNR_dB,tBer_QAM,'-*r','linewidth',3);
hold on
semilogy(SNR_dB ,BER);
grid on
